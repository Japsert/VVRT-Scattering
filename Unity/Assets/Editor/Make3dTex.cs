...
namespace Editor
{
    public class Make3dTex : MonoBehaviour
    {
        ...
        private static void CreateTexture3D(String filepath, String filename, int xsize, int ysize, int zsize)
        {
            //configure file path
        ...
        private static float[,,] GetGridFromFile(string path, int sizeX, int sizeY, int sizeZ)
        {
            ...
            BinaryReader binReader =
                new BinaryReader(File.Open((Application.streamingAssetsPath + path), FileMode.Open));
        ...
        private static Color CentralDiffColor(float[,,] grid, int x, int y, int z, int xsize, int ysize, int zsize,
            int delta)
        {
